...
function PlaylistScreen() {
  ...
  return (
    <Screen>
      ...
      {/* ── Shared by ── */}
      <div className="animate-rise mb-4 flex items-center gap-2">
      ...
      {PLAYLIST.length === 0 ? (
        <div className="flex flex-col items-center gap-3 py-20 text-center">
          ...
          <p className="text-sm text-muted-foreground">
            Add the songs that belong to the two of you
          </p>
        </div>
      ) : (
      <>
      ...
      <section className="animate-rise flex flex-col items-center" style={{ animationDelay: "60ms" }}>
        <img
          src={album}
          alt={`${PLAYLIST[index]?.title ?? ""} artwork`}
          width={768}
          height={768}
        ...
        />
        <h2 className="mt-6 text-xl font-semibold">{PLAYLIST[index]?.title}</h2>
        <p className="text-sm text-muted-foreground">{PLAYLIST[index]?.artist}</p>
      ...
      <h3 className="mt-8 mb-3 text-sm font-semibold tracking-wide text-muted-foreground uppercase">Up next</h3>
      <ul className="space-y-2">
        {PLAYLIST.map((t, i) => (
          <li key={t.title}>
            <button onClick={() => setIndex(i)}
              className={`glass flex min-h-14 w-full items-center gap-3 rounded-2xl px-3 py-2 text-left transition-transform active:scale-[0.98] ${i === index ? "ring-1 ring-primary/40" : ""}`}>
              <span className="w-5 text-center text-xs text-muted-foreground">{i + 1}</span>
              <span className="flex-1">
                <span className={`block text-sm font-medium ${i === index ? "text-primary" : ""}`}>{t.title}</span>
